...
impl Client {
        ...
        if message.is_type("act") && message.is_state("created") {
            match self.actions.get(&message.key) {
                Some(action) => {
                    ...
                    let state = executor.complete(&message.proc_id, &message.id, &outputs)?;
    ...
    pub fn action1(_inputs: &Vars) -> Vars {
        ...
        vars.insert("result".to_string(), json!(100));

        vars
    }
    pub fn action2(inputs: &Vars) -> Vars {
        let result = inputs.get("result").unwrap().as_i64().unwrap();

        let mut vars = Vars::new();
        vars.insert("uid".to_string(), json!("u3"));

        // change the result var again
        vars.insert("result".to_string(), json!(result * 2));
    ...
    pub fn action4(inputs: &Vars) -> Vars {
        println!("result:{:?}", inputs.get("result"));

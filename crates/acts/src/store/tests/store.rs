...
async fn store_model_query_by_offset_count() {
    ...
    let q = Query::new()
        .filter(Filter::and().expr(Expr::eq("create_time", create_time)))
        .offset(0)
        .limit(5);
...
async fn store_package_query_by_offset_count() {
    ...
    let q = Query::new()
        .filter(Filter::and().expr(Expr::eq("create_time", 100)))
        .offset(0)
        .limit(5);
...
async fn store_package_query_by_cond_or() {
    ...
    let q = Query::new().offset(0).limit(100).filter(
        Filter::and().expr(Expr::eq("create_time", 300)).push(
            Filter::or()
...
async fn store_package_query_by_order() {
    ...
    let q = Query::new()
        .offset(0)
        .limit(100)
        .filter(Filter::and().expr(Expr::eq("create_time", 400)))
        .order("timestamp", Sort::Desc);
    let ret = store.packages().query(&q).unwrap();

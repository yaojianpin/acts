...
use crate::{
    ...
    event::MessageState,
    store::query::*,
    utils::test::{USES_IRQ, create_proc},
    utils::{self, consts},
};
    ...
    emitter.on_message(move |msg| {
        ...
        async move {
            ...
            if msg.is_params_key("act2") && msg.is_state(MessageState::Created) {
                ...
                if let Some(aid) = aid {
                    let action = Action::new(&msg.pid, &aid, EventAction::Cancel, options);
                    s.do_action(&action).await.unwrap();
                }
            }
...
async fn sch_message_complete_message_in_store() {
    ...
    e2.runtime().launch(&proc).await.unwrap();
    let ret = tx.recv().await;
    e2.runtime().cache().flush().await.unwrap();
...
async fn sch_messages_not_removed_when_completed_in_store() {
    ...
    // Completion is signalled from the ack channel's own `on_complete`
    // handler: `deliver` stores the canonical message row before invoking the
    // handler, so when this fires the row is durable. The default channel
    // must not race it — handlers of the same event across channels have no
    // ordering guarantee (each handler is its own awaitable future).
    let options = ChannelOptions {
        id: "e1".to_string(),
        ack: true,
        ..Default::default()
